...
public class DeterministicFallbackAiProvider implements AiProvider {
    ...
    public ClinicalAiResult generateClinicalAnalysis(String systemPrompt, String userPrompt, String modelId) {
        ...
        if (textLower.contains("alt") || textLower.contains("ast") || textLower.contains("men gan") || textLower.contains("gan mat") || textLower.contains("tieu hoa")) {
            ...
            result.setPlainLanguageExplanation("Chỉ số men gan của bạn đang tăng báo hiệu gan đang bị tổn thương nhẹ. Bạn nên kiêng rượu bia và khám chuyên khoa Tiêu Hóa - Gan Mật sớm.");
            result.setDoctorRecommendationReason("Bác sĩ chuyên khoa Tiêu Hóa - Gan Mật có kinh nghiệm hạ men gan và tầm soát viêm gan vi rút hiệu quả.");
        } else if (textLower.contains("cholesterol") || textLower.contains("triglyceride") || textLower.contains("lipid") || textLower.contains("tim mach") || textLower.contains("mach vanh")) {
        ...
        } else if (textLower.contains("glucose") || textLower.contains("duong huyet") || textLower.contains("tieu duong") || textLower.contains("hba1c")) {
            result.setRecommendedSpecialtySlug("endocrinology");
            result.setRecommendedSpecialtyName("Endocrinology (Nội Tiết - Đái Tháo Đường)");

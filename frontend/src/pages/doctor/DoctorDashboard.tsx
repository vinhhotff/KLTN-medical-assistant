...
export const DoctorDashboard: React.FC = () => {
  ...
  const handleCreateFollowUp = async () => {
    ...
    try {
      ...
      const scheduledStart = `${followUpDate}T${followUpTime || '09:00'}:00`;
          ...
          <div className="bg-white w-full max-w-4xl rounded-3xl border border-slate-200 shadow-2xl overflow-hidden max-h-[92vh] flex flex-col animate-scaleUp">
            ...
            {activeEncounterTab === 'EMR' && (
              <form onSubmit={handleSubmitEncounter} className="p-6 overflow-y-auto space-y-6 flex-1 text-xs">
                ...
                {/* Banner: Linked AI Triage Session from Booking */}
                {(activeEncounterAppointment?.triageSessionId || activeEncounterAppointment?.triageSbarSummary) && (
                  <div className="p-4 bg-gradient-to-r from-amber-50 to-orange-50/40 border border-amber-200 rounded-2xl flex flex-wrap items-center justify-between gap-3 text-xs shadow-2xs">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 flex items-center gap-2">
                          <span>Phiên Phân Luồng AI Triage:</span>
                          {activeEncounterAppointment.triageUrgencyLevel && (
                            <span className={`px-2 py-0.5 rounded-lg border font-mono font-bold text-[11px] ${
                              activeEncounterAppointment.triageUrgencyLevel === 'EMERGENCY'
                                ? 'bg-rose-100 text-rose-800 border-rose-300'
                                : activeEncounterAppointment.triageUrgencyLevel === 'URGENT'
                        ...
                        {activeEncounterAppointment.triageSbarSummary && (
                          <p className="text-[11px] text-slate-700 mt-1 line-clamp-2 max-w-xl">
                            {activeEncounterAppointment.triageSbarSummary}
                          </p>
                        )}
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="font-bold text-slate-700 block mb-1">Hẹn Tái Khám (Ngày & Giờ)</label>
                    <div className="space-y-1.5">
                      <div className="grid grid-cols-2 gap-1.5">
                        <input
                          ...
                          className="w-full px-2.5 py-2 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-teal-500"
                        />
                        <input
                          type="time"
                          value={followUpTime}
